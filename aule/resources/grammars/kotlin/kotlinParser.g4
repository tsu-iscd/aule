...
functionTypeNoReceiver
  : '(' (functionType_paramOrType (',' functionType_paramOrType)*)? ')' '->' type_
  ;
...
getter
  : modifiers 'get'
  | modifiers 'get' '(' ')' (':' type_)? functionBody
  ;
...
functionDeclaration
  : modifiers 'fun' typeParameters?
      (type_ '.' | annotations)?
      SimpleName
      typeParameters? valueParameters (':' type_)?
      typeConstraints
      functionBody?
      SEMI*
  ;
...
namedInfix
  ...
  | elvisExpression (isOperation type_)?
  ;
